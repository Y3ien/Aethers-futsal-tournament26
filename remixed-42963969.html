...
<div class="panel">
  ...
  <div id="panel-register">
    ...
    <!-- Removed notice (hidden by default) -->
    ...
    <!-- Registered notice (hidden by default) -->
...
<script>
// ── CONSTANTS ──
const STORE_KEY   = "aethers_subs_v1";
...
// ── STORAGE (localStorage – works in real browsers) ──
function getSubs()      { try { return JSON.parse(localStorage.getItem(STORE_KEY)) || []; } catch { return []; } }
function saveSubs(a)    { localStorage.setItem(STORE_KEY, JSON.stringify(a)); }
...
// ── FORM VIEWS ──
function showForm()     { document.getElementById("reg-form").style.display=""; document.getElementById("removed-notice").style.display="none"; document.getElementById("registered-notice").style.display="none"; }
function showRemoved()  { document.getElementById("reg-form").style.display="none"; document.getElementById("removed-notice").style.display=""; document.getElementById("registered-notice").style.display="none"; }
...
// ── VALIDATION ──
function setErr(id, msg)   { const el=document.getElementById("err-"+id); el.textContent=msg; el.style.display=""; const inp=document.getElementById("f-"+id)||document.getElementById("f-"+id.replace("err-","")); if(inp){ inp.classList.add("has-err"); } }
function clearErr(id)      { const el=document.getElementById("err-"+id); if(el){ el.style.display="none"; } const inp=document.getElementById("f-"+id); if(inp) inp.classList.remove("has-err"); }
...
function submitForm() {
  ...
  try {
    const sub = {
      ...
      instagram: document.getElementById("f-ig").value.trim(),
      photo: photoData.src,
      submittedAt: Date.now(),
    };
    const all = getSubs(); all.push(sub); saveSubs(all); saveMy(sub);
    showRegistered(sub);
...
function withdraw() {
  ...
  const my = getMy();
  if (my) { saveSubs(getSubs().filter(s => s.id !== my.id)); clearMy(); }
  if (timerInterval) clearInterval(timerInterval);
  photoData = null;
...
function renderPlayers() {
  ...
  list.innerHTML="";
  if (!subs.length) { list.innerHTML='<div class="empty">No registrations yet.</div>'; return; }
    ...
    group.forEach(sub => {
      ...
      card.innerHTML=`
        ...
        <div class="info">
          ...
          <div class="pdate">${new Date(sub.submittedAt).toLocaleDateString()}</div>
        </div>
        <div class="actions">
          <button class="btn-card-view" onclick="openModal('${sub.id}')">View</button>
          <button class="btn-card-remove" onclick="removePlayer('${sub.id}')">Remove</button>
        </div>`;
...
function removePlayer(id) {
  const subs = getSubs(); const sub = subs.find(s => s.id===id);
  if (!sub) return;
  if (!confirm(`Remove ${sub.name}'s registration? They will be notified.`)) return;
...
function openModal(id) {
  ...
  document.getElementById("modal-body").innerHTML=`
    <div class="info-row"><div><div class="info-label">Submitted</div><div class="info-value">${new Date(sub.submittedAt).toLocaleString()}</div></div></div>
